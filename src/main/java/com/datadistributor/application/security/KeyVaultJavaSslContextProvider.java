  ...
  private Optional<SSLContext> loadFromKeyVault() {
    try {
      ...
      if (vaultUrl == null || vaultUrl.isBlank() || certificateName == null || certificateName.isBlank()) {
        log.warn("KeyVault Java SSL context not loaded: vaultUrl or certificateName missing");
        return Optional.empty();
      }
      ...
      log.info("Java SSL context loaded from Key Vault certificate {} for Feign clients", certificateName);
      return Optional.of(context);
    } catch (Exception ex) {
      log.error("Failed to load Java SSL context from Key Vault: {}", ex.getMessage(), ex);
      return Optional.empty();
    }
  }
}
