...
## Recent Updates
- **CET scheduler**: `DialSignalDataProviderScheduler` now uses `zone = "Europe/Amsterdam"` so the cron runs at the intended local time, and `GlobalTimeZoneConfig` pins the JVM default timezone to CET for any code that still relies on `LocalDate.now()` without an injected `Clock`.
- **Entity rename**: CEH mappings now live in `CehResponseInitialEventEntity`, so JPA repositories, adapters, and tests use the clearer name.

## Release Notes
| Version | Date | Highlights |
|---------|------|------------|
| 0.1.0 | 2025-12-05 | Baseline: hexagonal layout, Feign/WebClient delivery, schedulers + retry, Azure Blob report seam, CET-aware scheduler + timezone config, CEH entity rename, tests.
